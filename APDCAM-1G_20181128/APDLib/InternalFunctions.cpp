...
bool GetInfo(CAPDClient *client, APDInfo *apdInfo)
{
    ...
    retVal=GetHV1Max(client,&apdInfo->hv1Max);

    if(!retVal)
        return retVal;

    retVal=GetHV2Max(client,&apdInfo->hv2Max);
...
bool GetStatus(CAPDClient *client, APDStatus *apdStatus)
{
    ...
    retVal = GetFactoryData(client, &apdFactory);
    ...
    apdStatus->highVoltages[0]=0.0;
    apdStatus->highVoltages[1]=0.0;
    apdStatus->highVoltages[2]=0.0;
    apdStatus->highVoltages[3]=0.0;
    ...
    {
        apdStatus->highVoltages[0] = (double) binValues[0] * inputHvCalib1;

    }
    else if(apdFactory.productCode[0]=='M' && apdFactory.productCode[1]=='P')
    {

        double C = 2.15288;  //Only used for reading voltage
        double D = 0.0236476; //Only used for reading valtage, different from inputHvCalib2
        double x = (double) binValues[1] * D;

        apdStatus->highVoltages[0] = x+C;
    }
