...
ADT_RESULT APDCAM_GetHV(ADT_HANDLE handle, double *highVoltage1, double *highVoltage2, int *state)
{
    ...
    *highVoltage2 = (double) binValue2 * inputHvCalib2;
...
ADT_RESULT APDCAM_GetStatus(ADT_HANDLE handle, APDStatus *apdStatus)
{
	...
	//GetHVState(WorkingSet.client, &systemStatus->HVSate);
    ADT_RESULT R = APDCAM_GetHV(handle, &systemStatus->HighVoltages[0], &systemStatus->HighVoltages[1], &systemStatus->HVSate);
	if(R!=0)
		res = ADT_ERROR;

	int mode=0;
	R = APDCAM_GetShutterMode(handle, &mode);
	if(R!=0)
		res = ADT_ERROR;

	systemStatus->ShutterState=mode;

	int calibLight=0;
	R = APDCAM_GetCalibLight(handle, &calibLight);
	if(R!=0)
		res = ADT_ERROR;

    systemStatus->CalibrationLigth=(unsigned short)calibLight;*/
	...
	// Fan state?
	unsigned char ErrorCode; // // offs 0x86 - 8 bit
...
ADT_RESULT APDCAM_GetConfig(ADT_HANDLE handle, APDStr *apdStr)
{
    ...
    apdStr->hv2 = (double)binValue2 * inputHvCalib2;
