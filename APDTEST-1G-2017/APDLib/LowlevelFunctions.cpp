...
bool WritePDI(CAPDClient *client, unsigned char address, unsigned long subaddress, unsigned char* buffer, int noofbytes, unsigned long ip_address_h, unsigned short ip_port_h, int timeout)
{
	...
	do
	{
		...
		READPDI cmdr;
		cmdr.instruction.Prepare((unsigned char)address, subaddress, (unsigned short)noofbytes);
...
bool WritePDISafe(CAPDClient *client, unsigned char address, unsigned long subaddress, unsigned char *buffer, int noofbytes, unsigned long ip_address_h, unsigned short ip_port_h, int timeout)
{
    bool ret = false;

    int attempts = 10;
    bool valuesMatch = false;
    unsigned char *buffer2=new unsigned char[noofbytes];

    ret=WritePDI(client, address, subaddress, buffer, noofbytes);
    if(!ret)
        return  ret;
    ...
    do
    {
        bool ret=ReadPDI(client, address, subaddress, buffer2, noofbytes);
        ...
        for(int i=0;i<noofbytes;i++)
        {
            if(buffer[i]==buffer2[i])
            {
               valuesMatch = true;
               ret = true;
               //cout << "Write PDISafe: Values match:" << endl;
            }
            else
            {
               valuesMatch = false;
               ret = false;
               //cout << "Write PDISafe: Values NOT match:" << endl;
            }
        }
        ...
        {
            attempts--;
            //cout << "Write PDISafe: Remaining attempts:" << attempts << endl;
            Sleep(1);
        }

    } while (!valuesMatch && attempts > 0);
...
bool SendAck(CAPDClient *client, unsigned char* buffer, unsigned char ackType)
{
    bool res = true;
    ...
    clientContext->bufferLength = 509;
    clientContext->pBuffer = buffer;


    SENDACK cmd;
    cmd.instruction.Prepare(ackType);
